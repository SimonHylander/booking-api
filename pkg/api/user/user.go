...
import (
	...
	"github.com/simonhylander/booker/pkg/utl/query"
)

// Create creates a new user account
func (u User) Create(c echo.Context, req booker.User) (booker.User, error) {
	if err := u.rbac.AccountCreate(c, req.RoleID, req.CompanyID, req.LocationID); err != nil {
		return booker.User{}, err
	}
...
func (u User) List(c echo.Context, p booker.Pagination) ([]booker.User, error) {
	au := u.rbac.User(c)
	q, err := query.List(au)
	if err != nil {
		return nil, err
...
func (u User) View(c echo.Context, id int) (booker.User, error) {
...
func (u User) Update(c echo.Context, r Update) (booker.User, error) {
	if err := u.rbac.EnforceUser(c, r.ID); err != nil {
		return booker.User{}, err
	}

	if err := u.udb.Update(u.db, booker.User{
		Base:      booker.Base{ID: r.ID},
