...
import (
	...
	"github.com/simonhylander/booker/pkg/api/user/transport"
...
func TestCreate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				AccountCreateFn: func(c echo.Context, roleID booker.AccessRole, companyID, locationID int) error {
...

		{
			...
			udb: &mockdb.User{
				CreateFn: func(db orm.DB, usr booker.User) (booker.User, error) {
					usr.ID = 1
...
func TestList(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				UserFn: func(c echo.Context) booker.AuthUser {
					return gorsk.AuthUser{
						ID:         1,
						CompanyID:  2,
						LocationID: 3,
						Role:       booker.UserRole,
						Email:      "john@mail.com",
					}
				}},
			wantStatus: http.StatusForbidden,
			...
			rbac: &mock.RBAC{
				UserFn: func(c echo.Context) booker.AuthUser {
					return gorsk.AuthUser{
						...
						CompanyID:  2,
						LocationID: 3,
						Role:       booker.SuperAdminRole,
						Email:      "john@mail.com",
					}
				}},
			udb: &mockdb.User{
				ListFn: func(db orm.DB, q *booker.ListQuery, p booker.Pagination) ([]booker.User, error) {
...
func TestView(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (booker.User, error) {
					return booker.User{
						Base: booker.Base{
							ID:        1,
	...
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				...
				UpdateFn: func(db orm.DB, usr booker.User) error {
	...
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (booker.User, error) {
					return booker.User{
						Role: &booker.Role{
							AccessLevel: booker.CompanyAdminRole,
						},
					}, nil
				},
			},
		...
		},
		{
			...
			udb: &mockdb.User{
				...
				DeleteFn: func(orm.DB, booker.User) error {
					return nil
				},
			},
			rbac: &mock.RBAC{
