...
func (s Service) User(c echo.Context) booker.AuthUser {
	id := c.Get("id").(int)
	companyID := c.Get("company_id").(int)
	locationID := c.Get("location_id").(int)
	user := c.Get("username").(string)
	email := c.Get("email").(string)
	role := c.Get("role").(booker.AccessRole)
	return booker.AuthUser{
		ID:         id,
		Username:   user,
		CompanyID:  companyID,
...
func (s Service) EnforceRole(c echo.Context, r booker.AccessRole) error {
	return checkBool(!(c.Get("role").(booker.AccessRole) > r))
}
...
func (s Service) EnforceCompany(c echo.Context, ID int) error {
	...
	if err := s.EnforceRole(c, booker.CompanyAdminRole); err != nil {
		return err
	}
...
func (s Service) EnforceLocation(c echo.Context, ID int) error {
	...
	if err := s.EnforceRole(c, booker.LocationAdminRole); err != nil {
		return err
	}
...
func (s Service) isAdmin(c echo.Context) bool {
	return !(c.Get("role").(booker.AccessRole) > booker.AdminRole)
}

func (s Service) isCompanyAdmin(c echo.Context) bool {
	// Must query company ID in database for the given user
	return !(c.Get("role").(booker.AccessRole) > booker.CompanyAdminRole)
}
...
func (s Service) AccountCreate(c echo.Context, roleID booker.AccessRole, companyID, locationID int) error {
...
func (s Service) IsLowerRole(c echo.Context, r booker.AccessRole) error {
	return checkBool(c.Get("role").(booker.AccessRole) < r)
}